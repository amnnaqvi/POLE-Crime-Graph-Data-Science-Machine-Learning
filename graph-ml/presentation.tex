...
\begin{frame}[fragile]{Key Stats Query}
  \small
  This query is the turning point of the project: it checks whether person-crime prediction has enough labels to be fair or useful.
...
RETURN persons, crimes, count(r) AS observedPartyTo,
  ...
  \vspace{0.1cm}
  Result: only 55 observed \tinycode{PARTY\_TO} links out of 10,613,178 possible pairs, or 0.0005\%.
\end{frame}
...
\begin{frame}{Rejected Strategy: Person-Crime Prediction}
  ...
  \vspace{0.2cm}
  \textbf{Decision:} \tinycode{PARTY\_TO} stays in the graph as context, but we do not use it as the main write-back target. Write-back means creating a new relationship in Neo4j from the model output.
\end{frame}

\begin{frame}{Graph ML Design}
  \textbf{Final GML target:} link prediction (LP), meaning we predict missing relationships between existing nodes. Here, the target is missing social-family links for review, using the wider POLE graph as context.
...
\begin{frame}{Supervised GDS Pipeline}
  ...
  \begin{table}
    ...
    \begin{tabular}{p{0.28\textwidth}p{0.68\textwidth}}
      ...
      Model choice & Logistic Regression was tested as a simple baseline; the final demo pipeline keeps Random Forest (RF), a tree-ensemble model, because its scores had usable separation. \\
...
\begin{frame}[fragile]{Key ML Query}
  \small
  The model writes candidates only after the held-out score and score spread pass a simple gate.
...
\begin{frame}{ML Strategy Summary}
  ...
  \vspace{0.1cm}
  The point is not that every model wins. The point is that the graph tells us which tasks are credible and which ones are not.
\end{frame}
...
\begin{frame}{Model Comparison}
  ...
  \end{columns}
  \small
  This is a diagnostics chart, not a single leaderboard. AUCPR, F1, and accuracy answer different questions: ranking rare links, balancing class performance, and counting correct labels.
\end{frame}
...
\begin{frame}{Explainable GML: Common Neighbours}
  ...
  \vspace{0.2cm}
  This is easy to explain in a demo: two people are candidates only when they share multiple specific neighbours and have no existing social edge.
\end{frame}
